...
A. この端末に保存できなかったときは、「この端末に保存できませんでした」という警告メッセージが表示されます（以前のバージョンでは何も表示されず、再読み込みすると直近の変更が消えていました）。警告が出たら、画面を閉じる前に「ログイン」タブの移行用JSONを書き出すか、スプレッドシートへ送ってください。ブラウザの保存容量はおおむね5MB程度で、その多くは環境の時別データが占めます。不要になったハウスの環境データを「ハウス内環境」タブで消すと、容量を減らせます。保存に失敗している間は、同期カードにもその旨が表示されます。

...
| 注意点 | 内容 | 対策 |
|---|---|---|
| 保存が終わる前に閉じると、直前の変更が消えることがある | IndexedDB への保存は、裏で少し時間をかけて行われます（以前の localStorage は、押した瞬間に保存が終わっていました）。データが数十MBあると、1回の保存に1〜2秒かかることがあります。保存が終わる前にタブを閉じると、その保存は取り消されます。 | 記録や削除をしたら、数秒待ってから画面を閉じてください。 |

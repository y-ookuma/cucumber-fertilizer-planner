...
- LAI（葉面積指数、任意）は、記録として保存し、調査記録の表・CSV書き出し・カレンダーの予定に表示するだけです。**蒸散量などの計算には使っていません。** 「ハウス内環境」タブの蒸散量は、LAIを登録していても、常に定植からの経過日数で作物係数を見積もります（2章「ハウス内環境」の作物係数の説明を参照）。潅水量・液肥の実績は、このカードではなく「毎日の記録」タブで記録します。
...
「いまの生育」の生育調査でLAIを記録することはできますが、蒸散量の計算には使っていません。そのかわり、ハウス一覧に定植日を登録していれば、定植からの経過日数をLAIの代わりの目安として使い、作物係数を定植直後0.3から定植45日後1.0まで変化させます（2章「ハウス内環境」参照）。これにより、苗が小さい定植直後の蒸散量を過大評価しないようにしています。ただし実際の葉面積・仕立て方・摘葉の状況までは反映していないため、あくまで簡易な近似です。定植日を登録していないハウスでは、従来どおり葉が茂った状態（Kc=1.0固定）で計算します。

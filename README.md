...
- LAIの欄の下の「葉の測定からLAIを計算する（NARO式）」を開き、株の状態・葉数（1株平均）・最大葉の葉長と葉幅を入れると、LAIを計算してLAI欄に入れます（3章「葉の測定からLAIを計算する（NARO式）」参照）。計算には、ハウス一覧の栽植株数と栽培面積が必要です。計算したLAIは手で直すこともでき、保存されるのはLAI欄の値です。葉の測定値も一緒に保存します。潅水量・液肥の実績は、このカードではなく「毎日の記録」タブで記録します。
...
**測り方**
1. 生育調査の4〜6株で、着いている葉の数を数え、1株あたりの平均を入れます。
2. 同じ株で、いちばん大きい葉（生長点から10枚目より下の、よく展開した葉）の葉長と葉幅を測り、平均を入れます。
3. 「株の状態」を選びます。

| 株の状態 | 計算 | 使いどころ |
|---|---|---|
| 枝が伸びている（生長点付近の若い葉も数える） | NARO式そのまま（上の積分） | 主枝の摘心前など、生長点のある枝が葉を増やしている時期 |
...
### 単回帰
X（平均気温・昼温・夜温・DIF・相対湿度・飽差・CO₂濃度・積算PAR・受光量・蒸散量から1つ）と Y（茎径・節間長・開花数・着果数・収量・日次収量・潅水量から1つ）の組から、最小二乗法で
...
| ボタン | 内容 |
|---|---|
| 調査記録CSV | 生育調査の記録（集計値・生データ・開花数・着果数・収量入力値・収量採用値（毎日の記録優先）・乾物kg・持出N/P2O5/K2O・LAI・葉数・最大葉の葉長と葉幅・LAIの計算方法・メモ） |

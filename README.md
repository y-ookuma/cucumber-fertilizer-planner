...
東出ら（2012）の式2にならったものです。LAIは生育調査の記録を日ごとに補間した値を使うので、LAIを一度も記録していないハウスでは表示しません。表の下には、定植からの積算受光量（データのある日の合計）を、論文の摘心区の目安（定植40日で約200、98日で約800 mol/m²）と並べて表示します。論文では、定植40日後までの乾物生産は受光量で決まっていました。回帰分析の説明変数（X）としても選べます。
